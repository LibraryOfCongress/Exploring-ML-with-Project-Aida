...
class UNeXt(nn.HybridBlock):
    ...
    def __init__(self, _cfgs, _cardianlity, _basewidth, **kwargs):
        ...
        with self.name_scope():
            ...
            self.u3 = up_block(_ch*4, shrink=True)
            self.u2 = up_block(_ch*2, shrink=True)
            self.u1 = up_block(_ch, shrink=True)
            self.u0 = up_block(_ch//2, shrink=True)
            self.uc = up_block(_ch//4, shrink=True)

            self.conv = nn.HybridSequential()
            self.conv.add(nn.Conv2D(5,1))

...
class IoUMetric(mx.metric.EvalMetric):
    ...
    def update(self, batch, labels, preds, exist_cls):
        ...
        comp = (pl==l).astype(np.int).sum()
        
        confus_tb = confusion_mat_multiclass_2d(pl, l, self.nbcls)
        tp = np.diagonal(confus_tb)
        sum_row = confus_tb.sum(axis=0)
        sum_col = confus_tb.sum(axis=1)
        
        denominator = sum_row + sum_col - tp
        nb_valid = np.sum((denominator != 0).astype(np.int))
        
        ious = np.nan_to_num(tp/denominator)
        miou = ious.sum()/nb_valid
        ...
        cls_mask = np.zeros(self.nbcls)
        cls_mask[exist_cls] = 1

        self.acc = np.array(accs) * cls_mask
        self.macc = comp / np.prod(l.shape)
...
def confusion_mat_multiclass_2d(pred, label, nbCls):
    ...
    for i in range(row):
        for j in range(col):
            record[label[i,j].astype(np.int), pred[i,j].astype(np.int)]  += 1
